      ...
      const response = await fetch("https://api.openai.com/v1/chat/completions", {
          ...
          body: JSON.stringify({
            ...
            messages: [
              {
                role: "user",
                content: `${nameA}와 ${nameB}라는 사람이 있다. ${nameA}가 ${a} ${nameB}가 ${b} 누가 더 잘못했는가? 논리적으로 타당한 답을 하라. 답변에 이름과 잘못한 비율을 %로 표시한다. 답변 형식은 "${nameA}의 잘못한 비율 %, ${nameB}의 잘못한 비율 %, 3문장 내외로 근거 설명." 한쪽이 100% 잘못이라는 답변은 금지. 말투는 공손하게.`,
              },
            ],
            temperature: 0.6,
            max_tokens: 300,
