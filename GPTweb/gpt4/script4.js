document.querySelector("#home-button").addEventListener("click", function home() {
      ...
      .then(async function (dataUrl) {
        ...
        if (navigator.share) {
            try {
                ...
                let file = new File([blob], 'AI-jagang.png', {type: blob.type});
  
                await navigator.share({
                    title: 'AI jagangDucheon',
                    text: '누구 잘못인지 제대로 가려보자!',
                    url: 'https://sohyunsoo.xyz/GPTweb/gpt4/gpt4.html',
                    files: [file] 
                })
            ...
            textArea.value = 'https://sohyunsoo.xyz/GPTweb/gpt4/gpt4.html';
  ...
  async function chat(nameA, nameB, a, b) {
    const response = await fetch("/api/chat4", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ nameA, nameB, a, b }),
    });
  ...
  document.querySelector("#send-button").addEventListener("click", async function (event) {
    ...
    const answer = await chat(nameA, nameB, a, b);
