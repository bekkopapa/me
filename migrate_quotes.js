...
// serviceAccountKey.json 파일이 같은 폴더에 있어야 합니다.
...
const quotes = [
  ...
  { text: '마지막 임무가 끝이 났다다는 건, 내 품안의 엘리도, 아끼는 동료들도 잃을 필요가 없게 됐다는 의미다. 나는 뒤늦게 감정이 북받쳐 오르는 걸 느꼈다. 엘리가 우는 이유도 알 것 같았다. 눈물을 흘리지는 않았다. 다만 품안의 엘리를 더 꽉 끌어안았을 뿐이다. 이로써 나의 전쟁은 끝났다.', author: '-진 오시리스 / 프린테라' },
...
async function uploadQuotes() {
  ...
  quotes.forEach((quote, index) => {
    const docRef = db.collection('quotes').doc(`quote_${index}`);
    batch.set(docRef, quote);
  });

  await batch.commit();
  console.log(`${quotes.length}개의 명언이 Firestore에 성공적으로 저장되었습니다.`);
  process.exit();
}

uploadQuotes().catch(err => {
